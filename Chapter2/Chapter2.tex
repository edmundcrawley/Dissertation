\chapter{Consumption Heterogeneity: Micro Drivers and Macro Implications}
...
Most micro-empirical evidence on consumption behavior comes in the form of an estimate of the marginal propensity to consume out of a one time source of income over the following three months to one year. Table \ref{table:MPCLiterature} shows a selection of the population average estimates from the literature.\footnote{This table is adapted from \cite{carroll_distribution_2017}} Most of these studies do not have the power to say much if anything about heterogeneity within the population.
...
\begin{sidewaystable} \small
	...
	\begin{minipage}{\textwidth}
		...
		\input ./Chapter2/Paper/Tables/MPCLiterature.tex 
		}
...
where $N$, the number of years over which growth is measured, varies from 1 to 10. Our full identification will come from the fact that transitory income shocks make up a relatively large proportion of the variance of income growth over a short period, while permanent income shocks dominate the variance of income growth over a long period. Figure \ref{fig:GrowthReg} shows what we would expect to see under three well known models, as well as what we actually observe in the data. In a complete markets model in which all idiosyncratic shocks to income are insured against there is no relation between income growth and consumption growth, as represented by the blue horizontal line at zero. In the Solow growth model, and also in some old Keynesian models, households' expenditure is a constant proportion of income in that period, regardless of transitory shocks to income. The green horizontal line around 0.75 shows what we would see in a model of this type where households spend 75\% of their income each period. The red line shows the results for a typical buffer-stock saving model.\footnote{These regression results come from the benchmark model presented in section \ref{benchmark_model}, calibrated to the distribution of liquid wealth in Denmark.} In this model the regression of consumption growth on income growth over one year yields a relatively small number as households are able to self-insure against the transitory shocks that dominate at this frequency. As the time period over which income growth is measured increases the observed income growth is proportionally more permanent and self-insurance is not possible. The red line asymptotes towards 1.0 as N gets large.
	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.7]{./Chapter2/Paper/Figures/basic_regression_liquid_wealth_level_lincome_head.pdf}
...
Figure \ref{fig:TimeAgg} shows how the problem arises. The solid `Income flow' line shows the true income flow of a household who receives zero income throughout year 1, zero income for the first half of year 2, and then a constant income flow of 1.0 per year during the second half of year 2 and in year 3. The dashed line shows the observed \textit{total} income of the household in years 1, 2 and 3: zero in year 1, 0.5 in year 2 and 1.0 in year 3. The important thing to note is that despite there only being one `shock' to the income flow over the three year period, the na\"ive observer would assume there had been two shocks, one between years 1 and 2 and another between years 2 and 3. This effect is of particular importance to econometric techniques that make use of the auto-covariance structure of data processes. For example the first difference of a random walk in discrete time has no autocorrelation, but the first difference of a time-aggregated random walk in continuous time has an autocorrelation equal to 1/4. BPP use time aggregated income data and achieve identification of transitory variance precisely through the auto-covariance structure. This is why the problem is particularly pervasive for this methodology: in a simulated dataset where households follow the permanent income hypothesis, that is they respond one-for-one to shocks to permanent income but not at all to transitory income shocks, the estimate for the consumption response to transitory income shocks using the BPP methodology is \textit{negative} 0.6.\footnote{This is for a simulation in which permanent and transitory shock variances are equal and shocks are uniformly distributed over the year.} 
	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.5]{./Chapter2/Paper/Figures/TimeAggExample2.pdf} 
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.7]{./Chapter2/Paper/Figures/GenericTransitory.pdf} 
...
Our approach will be to extend the identification of income variance by using growth over 3, 4 and 5 years to also identify the covariance of income and consumption. In contrast to BPP, which assumes that consumption follows a random walk, we will instead assume that the impulse response to a transitory shock follows a generic path, $g(t)$, that, like the transitory income shock, has fallen to zero 2 years after the news of the shock. Figure \ref{fig:GenericTransitoryBPP} shows possible paths for both income and consumption, along with the alternative random walk impulse response of BPP. The best evidence for the speed at which the consumption response decays comes from \cite{gelman_what_2016} and \cite{fagereng_mpc_2016}, both of which show that the response has entirely or almost entirely decayed 2 years after the shock. In section \ref{Consumption_persistence} we will show how this assumption may potentially bias the transitory consumption response down, but that this bias is small, especially for all but the most liquid households. We will maintain the assumption from BPP that the consumption response to a permanent shock to income follows a random walk proportional to the permanent shock. Under these assumptions the instantaneous flow of consumption is given by:	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.6]{./Chapter2/Paper/Figures/GenericTransitoryConsumptionWithBPP.pdf} 
		\caption{Generic Transitory Shock Impulse Response}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.8]{./Chapter2/Paper/Figures/consumption_measures.png} 
...
\begin{center} 
	...
	\input ./Chapter2/Paper/Tables/summary_statistics.tex 
...
\begin{figure}
	\centering
	\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/VarianceByLiquidWealth_level_lincome_head.pdf}
	\centering
	\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXByLiquidWealth_level_lincome_head.pdf}
...
\begin{figure}
	\centering
	\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/VarianceByNetWealth_level_lincome_head.pdf}
	\centering
	\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXByNetWealth_level_lincome_head.pdf}
...
\begin{figure} 
\begin{centering}
	\includegraphics[scale=0.85]{./Chapter2/Paper/Figures/MPXByUREdetailsPaper_level_lincome_head.pdf} \\
	\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXByNNP_level_lincome_head.pdf}
	\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXByIncome_level_lincome_head.pdf}
...
\begin{center}
	\label{table:aggElas}
	\input ./Chapter2/Paper/Tables/URE_NNP_table.tex
...
\begin{center}
	...
	\input ./Chapter2/Paper/Tables/sufficient_stats.tex
\end{center}
...
\begin{figure} 
		\centering
		\includegraphics[scale=0.45]{./Chapter2/Paper/Figures/benchmark_Lorenz.png}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/CSTW_tran_denmark.pdf}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/CSTW_tran_denmark_pref.pdf}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/CSTW_perm_denmark.pdf}
\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/CSTW_perm_denmark_pref.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.6]{./Chapter2/Paper/Figures/DurableBias.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXByDurables_nodurableproxy.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/DecayBias.pdf}
...
\subsection{Identification with Time Aggregation}\label{sec:Identification}

\input ./Chapter2/Paper/Appendices/Identification3.tex

%\input ./Chapter2/Paper/Appendices/Identification2.tex
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.6]{./Chapter2/Paper/Figures/VarianceByAge_level_lincome_head.pdf} 
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.6]{./Chapter2/Paper/Figures/MPXByAge_level_lincome_head.pdf} 
		\caption{MPX by Age}
		\label{fig:MPXByAge}
	\end{centering}
\end{figure}

\subsection{The Danish Mortgage Market} \label{mortgage_market}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.7]{./Chapter2/Paper/Figures/mortgage_by_type.png} 
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.7]{./Chapter2/Paper/Figures/mortgage_by_maturity.png} 
...
\subsection{Details on the Calculation of NNP and URE}
...
\subsubsection{NNP and URE for Households}
...
\subsection{Persistent Consumption Response} \label{consumption_persistence_appendix}
\subsubsection{Details on Section \ref{Consumption_persistence} Simulations}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/DecayBias_phi.pdf}
		\caption{Bias from Persistent Consumption}
		\label{fig:DecayBias_phi}
	\end{centering}
\end{figure}

\subsubsection{Persistent Consumption in the Preference Shock Model}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPC_accuracy.pdf}
...
\subsubsection{Estimates Using Different Values of $N$}
\begin{center}
	\captionof{table}{$\psi$ Estimates Using Different $N$}
	\label{table:psi_estimates}
	\input ./Chapter2/Paper/Tables/Psi_array_empirical.tex
\end{center}
...
\begin{center}
	\input ./Chapter2/Paper/Tables/beta_laborsupply50.tex	
	\input ./Chapter2/Paper/Tables/TranShk_laborsupply50.tex		\captionof{table}{Fitted Discount Factors and Transitory Shock Standard Deviation}
	\label{table:fitted_beta_and_transtd}
\end{center}

\begin{center}
	\input ./Chapter2/Paper/Tables/phi_laborsupply50.tex	
	\input ./Chapter2/Paper/Tables/c_std_laborsupply50.tex		\captionof{table}{Simulation Estimates of $\phi$ and Consumption Growth Standard Deviation}
	\label{table:phi_laborsupply}
\end{center}

\begin{center}
	\input ./Chapter2/Paper/Tables/mpc_laborsupply50.tex
	\input ./Chapter2/Paper/Tables/psi_laborsupply50.tex		\captionof{table}{Simulation Estimates of MPC and $\psi$}
...
\subsection{RIP or HIP Income Process?} \label{rip_hip_appendix}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/IncreasingDiff_level_lincome_head.pdf}
		\caption{Variance and Covariance with Years of Growth}
		\label{fig:increasing_diff}
	\end{centering}
\end{figure}

\subsection{Time Varying Risk} \label{time_varying_risk_appendix}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/IncomeGrowthStd.pdf}
...
\begin{figure}
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/Robust_tranMPX_liquidwealth.pdf}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/Robust_tranMPX_URE.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/Logs_tranMPX_liquidwealth.pdf}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/Logs_tranMPX_URE.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/total_tranMPX_URE.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/DivPerm_tranMPX_liquidwealth.pdf}
...
\subsection{Distribution of Permanent MPX by NNP, URE and Income} \label{PermMPXbyURENNP}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXBypermURE_level_lincome_head.pdf}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXBypermNNP_level_lincome_head.pdf}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/MPXBypermIncome_level_lincome_head.pdf}
		\caption{MPX Distribution by URE, NNP and Income}
		\label{fig:MPCAuclert_perm}
	\end{centering}
\end{figure}

\subsection{Intra-household Income Insurance} \label{Insurance}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{./Chapter2/Paper/Figures/Spouse_tranMPX_liquidwealth.pdf}
